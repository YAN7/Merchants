...
class MyDevices extends Component {
  ...
  // 请求设备列表
  getDevicesList() {
    Http.postWithAuth("/store/?method=device.getList", {}, re=> {
  ...
  _renderList = (item)=> (
    <TouchableOpacity
      onPress={()=> this.props.navigation.navigate("DevicesDetail", {device_id: item.device_id, modelName: item.model_name})}
      key={item.device_id}
    ...
    >
      ...
      <View style={[BORDER_BOTTOM, styles.list_right_wrapper]}>
        <View>
          <Text style={{color: '#333'}}>{item.model_name}</Text>
